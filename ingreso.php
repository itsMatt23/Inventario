...
// Procesar el formulario de búsqueda de proveedor
$proveedor = null;
if (isset($_GET['cedula'])) {
    $cedula = $_GET['cedula'];

    // Consulta para obtener los datos del proveedor
    $consulta = "SELECT * FROM proveedores WHERE cedula = '$cedula'";
...
<div class="container">
    ...
    <form method="get" action="ingreso.php" class="mb-4">
    ...
    <?php if (isset($proveedor) && $proveedor): ?>
        ...
        <p>Apellido: <?php echo $proveedor['Apellido']; ?></p>
    ...
    <?php endif; ?>
                ...
                <div class="col-md-4 mb-4">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <form onsubmit="agregarAlCarrito(event, <?php echo $producto['ProductoID']; ?>, '<?php echo $producto['Nombre']; ?>')">
                                <div class="input-group">
                                    <div class="input-group-prepend">
                                        <button type="button" class="btn btn-outline-secondary" onclick="decrementarCantidad(<?php echo $producto['ProductoID']; ?>)">-</button>
                                    </div>
                                    <input type="number" id="cantidad_<?php echo $producto['ProductoID']; ?>" class="form-control" min="1" value="1" required readonly>
                                    <div class="input-group-append">
                                        <button type="button" class="btn btn-outline-secondary" onclick="incrementarCantidad(<?php echo $producto['ProductoID']; ?>)">+</button>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label for="precio_<?php echo $producto['ProductoID']; ?>">Precio de Compra:</label>
                                    <input type="number" id="precio_<?php echo $producto['ProductoID']; ?>" class="form-control" step="0.01" min="0.01" required>
                                </div>
        ...
        <button class="btn btn-success" onclick="procesarCompra()">Procesar Ingreso</button>
...
<script>
        ...
        function agregarAlCarrito(event, productoID, nombre) {
        ...
        function eliminarProducto(index) {
            ...
            actualizarTablaCarrito(); // Actualizar la tabla del carrito después de eliminar
        }
        ...
        function procesarCompra() {
            ...
            if (confirm('¿Estás seguro de procesar el ingreso?')) {
            const xhr = new XMLHttpRequest();
            xhr.open('POST', 'php/procesar_ingreso.php', true);
            ...
            xhr.send(JSON.stringify({ carrito, proveedorCedula: '<?php echo $proveedor['Cedula']; ?>' }));
        }
    }
